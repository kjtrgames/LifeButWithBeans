...
def render():
    GameCoreData.screen.fill(HelpfulFuncs.htColorCodeTranslater("#00ff00"))
    _cameraMover()
...
def _plotsRender():
    ...
    for ix in range(CoreData.getMapSizeX()):
        for iy in range(CoreData.getMapSizeY()):
            CoreData.getPlotWithPos(ix, iy).render(x=ix, y=iy, cx=cx, cy=cy, cz=cz, size=1600//cz)#ReturneD Function

def _guiRender():
    sipy = max(0, (GameCoreData.itembar_height-GameCoreData.itembar_width)//2)
    #selected_item_pos_y
    _itembarRender(sipy); _itemsRender(sipy)

def _itembarRender(sipy):
    Renders.rectRender(0, 0, GameCoreData.itembar_width , GameCoreData.display_size_y, HelpfulFuncs.htColorCodeTranslater("#434"))
    Renders.rectRender(0, 0, GameCoreData.display_size_x, GameCoreData.itembar_height, HelpfulFuncs.htColorCodeTranslater("#767"))
    Renders.rectRender(0,sipy,GameCoreData.itembar_width, GameCoreData.itembar_width, HelpfulFuncs.htColorCodeTranslater("#ccc"))
